...
public class Locators_Concept {
	...
	public static void main(String[] args) throws InterruptedException {
		...
		/**
		 * 
		 * there're 4 different ways to create web elements they are shown below my
		 * personla approach is with by class
		 * 
		 * 
		 * in some application right click and open developer tool is not available for
		 * that u can use press function f12 button or view-->>developer-->>developer
		 * tool
		 * 
		 * 
		 ...
		 * 
		 * 
		 * we do all the action on web elelement not on by locator
		 * 
		 */
...
//		driver.findElement(By.className("login-password")).sendKeys("test@123");
		...
		// 4. xpath: -IV-- is the address of the element in HTML DOM but its not an
		// attribute
		/**
		 * XML Path Language XPath stands for XML Path Language. It uses a non-XML
		 * syntax to provide a flexible way of addressing (pointing to) different parts
		 * of an XML document. It can also be used to test addressed nodes within a
...
//		driver.findElement(By.xpath("//*[@id='loginBtn']")).click();
		...
		/**
		 * linktext associated with a tag and sometimes a tag can be parent also
		 * 
		 * hypertext reference href---->>>stands for hypertext reference. It's the
		 * source of the file used by the tag. You can use both not only when connecting
		 * an external css file, also for using <a> tags,for a regular hyperlink.
		 */
		...
		// 8. tagName: --V
		/**
		 * tag name not all the time is usefull because tag name can be multiple tags
		 * with same tags but sometimes in some web pages header is with html tag name
		 * can be found
		 */
